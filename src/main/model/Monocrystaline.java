package model;

import java.lang.Math;

// creates a Monocrystalline solar panel
public class Monocrystaline extends SolarPanelType  {

    // effects : sets solar type to Monocrystalline
    public Monocrystaline() {
        super("Monocrystalline");
    }

    // modifies : this
    // effects : sets the weather condition to 0, if winter, 1 if summer, 0.5 if Spring/ Autumn
    @Override
    public void setWeather(String weather) {
        if (weather.equals("Winter")) {
            this.weatherCondition = 0.2;
        } else if (weather.equals("Summer")) {
            this.weatherCondition = 1;
        } else  {
            this.weatherCondition = 0.5;
        }
    }
}